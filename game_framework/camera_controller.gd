...
export var max_shark = 1
export var is_shark_locked: bool =  true
...
export var shark_speed: float = 0.05
...
func _ready():
	...
	timer.connect("timeout", self, "_on_SharkTimer_timeout")
	timer.wait_time = shark_speed
...
func ser_current_camera(camera: Camera2D):
...
func shark_screen(shake: float):
	current_shake = clamp(current_shake, 0, max_shark)
	_move_camera_to(_get_random_position())

func _screen_shark_loop():
	if not is_shark_locked:
